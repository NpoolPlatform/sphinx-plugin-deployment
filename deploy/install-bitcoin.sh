...
function install_btc() {
  ...
  sed -i 's/#rpcpassword=.*/rpcpassword='$RPC_PASSWORD'/g' /root/.bitcoin/bitcoin.conf
  sed -i 's/#rpcuser=.*/rpcuser='$RPC_USER'/g' /root/.bitcoin/bitcoin.conf
  if [ "$COIN_NET" == "main" ]; then
    bitcoind -daemon -datadir=$DATA_DIR/bitcoin -rpcuser=$RPC_USER -rpcpassword=$RPC_PASSWORD >> $LOG_FILE 2>&1
  else
    bitcoind -regtest -addresstype=legacy -daemon -datadir=$DATA_DIR/bitcoin -rpcuser=$RPC_USER -rpcpassword=$RPC_PASSWORD >> $LOG_FILE 2>&1
    echo "rpcwallet=my_wallet" >> /root/.bitcoin/bitcoin.conf
  fi
}
